...
def calculate_color_distortion(image1, image2):
    ...
    color_distortion = color_distortion / np.max(color_distortion)

    # Create the heatmap
    plt.figure(figsize=(10, 8))
    sns.heatmap(color_distortion, cmap='viridis', annot=False, fmt=".2f")
...
def output_color_distortion(ground_truth_folder, generated_folder):
    ...
    for i in range(len(ground_truth_images)):
        calculate_color_distortion(ground_truth_images[i], generated_images[i])

    
output_color_distortion('Ground_Truth', 'Predicted_Image')
